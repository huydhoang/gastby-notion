...
def process_block(block, text_prefix=''):
    ...
    for content in block.children:
        ...
        if content.type == 'header':
        ...
        elif content.type == 'image' and content.source is not None:
            ...
            match_meta = regex_meta.match(content.title)
            if match_meta:
                key = match_meta.group(1)
                value = match_meta.group(2)
...
def to_markdown(page_id, ignore):
    ...
    _slug = slugify(page_title)
    # text = ''

    # Handle frontmatter
    metas = [f"title: '{page_title}'"]
